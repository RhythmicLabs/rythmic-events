...
import type { ScheduledEventManager } from '@/scheduler/index';
...
export class TestSetup {
  ...
  /**
   * Create ScheduledEventManager for testing
   */
  static async createScheduledEventManager(
    options: {
      withCache?: boolean;
    } = {}
  ): Promise<ScheduledEventManager> {
    const { ScheduledEventManager } = await import('../../scheduler/index');
    const eventBus = await this.createEventBus({ withCache: options.withCache !== false });
    return new ScheduledEventManager(eventBus);
  }
